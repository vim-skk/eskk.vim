...
function! s:asym_filter.filter(stash) "{{{
    if !self.key_table.dispatch(a:stash.char, [a:stash])
        call s:handle_fallback(a:stash, self.table)
    endif
endfunction "}}}
...
function! eskk#_initialize() "{{{
    ...
    function! s:initialize_key_table()
        ...
        for sp_key in eskk#map#get_special_keys()
            let key = eskk#map#get_special_key(sp_key)
            let handler = eskk#map#get_special_key_handler(sp_key)
            if key != ''
            \   && (type(handler) == type('')
        ...
        let key = eskk#map#get_special_key('phase:henkan-select:delete-from-dict')
        if key != ''
